...
df_downsampled = df.iloc[::50]  # Selects every 10th row, starting from index 0 
...
re = np.mean(np.abs((actuals_inv - predictions_inv) / actuals_inv)) * 100  # Percentage form

print(f"Score (RMSE): {rmse:.4f}")
print(f"Score (MAE): {mae:.4f}")
print(f"Score (Relative Error %): {re:.2f}%")
